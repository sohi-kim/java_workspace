...
-- 		 2) DDL(정의) - 데이터를 저장할 테이블 또는 사용자 등등 오라클의 객체 생성/삭제 명령(create,alter,drop)
--		 3) DCL(제어) - 계정 등등 데이터를 다루는데 필요한 기능을 실행 명령(grant, revoke)

-- 테이블 생성하여 데이터 저장할 준비.
--   ㄴ 컬럼(열) , 로우(행)
-- 컬럼은 테이블이 저장할 데이터 항목이름과 형식지정 , 로우는 각 컬럼에 데이터를 저장한 하나의 개체

-- Data Define Language : 1. 테이블 생성
...
-- Data Manipulation Language 데이터조작언어 : 1. 테이블에 데이터 추가
